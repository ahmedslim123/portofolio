...
/**
 * The case-study room that opens behind a door.
 *
 * Split out of Projects.jsx so that framer-motion — which this is the only
 * remaining consumer of — is not part of the first-paint bundle. Projects.jsx
 * loads it with next/dynamic and prefetches it the moment a door is hovered or
 * pressed, so the chunk is there well before the 720ms door swing ends.
 *
 * Kept mounted once opened: AnimatePresence has to stay in the tree for the
 * closing animation to play.
 */

const EASE = [0.165, 0.84, 0.44, 1];

export const PHONE_MQ = "(max-width: 700px)";

// The thumbnail strip paints 90x58 chips; scripts/optimize-images.cjs writes a
// 240px `-thumb.webp` beside every gallery frame.
const chip = (src) =>
  typeof src === "string" && src.endsWith(".webp") ? src.replace(/\.webp$/, "-thumb.webp") : src;

// The 900px variant, for phones — see the <picture> below.
export const small = (src) =>
...
// Vimeo needs an API call for thumbnails, so vimeo media should ship a poster.
...
// A video "facade": poster + play button, with the heavy iframe mounted only
// once the visitor actually clicks.
...
// The phone version. Framer animates by writing inline styles on the main
// thread: traced on a phone, the full entrance rewrote the style attribute of
// eight elements every frame for ~700ms. Phones animate the container only, in
// opacity and translate — both compositor properties, where rotateY and scale
// would force a full-screen re-rasterisation at a 3x pixel ratio every frame.
...
export default function ProjectRoom({
  ...
  // parent's animation, so the staggered children stop being written every frame.
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    >
                      {EMBED_TYPES.has(current.type) ? (
                        ...
                        // <picture> rather than srcset/sizes: on a 3x phone the
                        // browser multiplies the slot width by the device pixel
                        // ratio and picks the 1600px file anyway. A media query
                        // is the only way to say "small screen, small file" and
                        // have it obeyed.
                        <picture>
                          <source
                            media={PHONE_MQ}
